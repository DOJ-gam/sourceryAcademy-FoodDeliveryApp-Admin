...
export default function AddFood() {
  ...
  const submitCategory = (e) => {
    ...
    axios.post(`http://127.0.0.1:8000/api/store-category`, data).then(res => {
  ...
  useEffect(() =>{
    async function getData(){
      const res =await axios.get(`http://127.0.0.1:8000/api/get-category`)
  ...
  const deleteCategory = (e, id) =>{
    ...
    axios.delete(`http://127.0.0.1:8000/api/delete-category/${id}`).then(res =>{
      if(res.data.status === 200){
        swal('Success', res.data.message, 'success');
